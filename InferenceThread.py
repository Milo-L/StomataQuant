...
class YOLOSegInferenceThread(QThread):
    ...
    def run(self):
        try:
            ...
            save_path = self.inference_settings.get("save_path", os.path.join(os.getcwd(), "OutPut"))
            ...
            predict_params = {
                ...
                "show_boxes": False,
                "retina_masks": True,
                "name": file_name + " generated by Ultralytics"
            }
            ...
            if self._is_interrupted:
                print("YOLOSegInferenceThread interrupted before processing")
                self.inferenceFinished.emit(None, self.file_path, "Operation canceled")
                return

            # 调用 YOLO 的 predict 方法，传入参数字典
            results = self.model.predict(**predict_params)
            if self._is_interrupted:
                print("YOLOSegInferenceThread interrupted after processing")
                self.inferenceFinished.emit(None, self.file_path, "Operation canceled")
                return
